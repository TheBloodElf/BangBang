...
//
//  Created by sho yakushiji on 4413/11/05.
//  Copyright (c) 4413年 CALACULU. All rights reserved.
//
...
- (void)initImageScrollView
{
    if(_scrollView==nil){
        ...
        if(self.navigationController){
            ...
            y = ([UIDevice iosVersion] < 7) ? y-44 : y;
        }
        else{
...
- (void)expropriateImageView
{
    ...
    [UIView animateWithDuration:kCLImageToolAnimationDuration
                     animations:^{
                         ...
                         CGFloat dy = ([UIDevice iosVersion]<7) ? 44 : 0;
...
- (void)swapNavigationBarWithEditting:(BOOL)editting
{
    ...
    if(editting){
        ...
        [UIView animateWithDuration:kCLImageToolAnimationDuration
                         animations:^{
                             self.navigationController.navigationBar.transform = CGAffineTransformMakeTranslation(0, -self.navigationController.navigationBar.height-44);
                             _navigationBar.transform = CGAffineTransformIdentity;
                         }
         ];
    }
...
- (IBAction)pushedDoneBtn:(id)sender
{
    ...
    [self.currentTool executeWithCompletionBlock:^(UIImage *image, NSError *error, NSDictionary *userInfo) {
        if(error){
            ...
            UIAlertAction *okBnt = [UIAlertAction actionWithTitle:@"ok" style:UIAlertActionStyleCancel handler:nil];
            [alertVC addAction:okBnt];
