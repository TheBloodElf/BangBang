...
+ (NSURLSessionDataTask*)getQqUuidWithToken:(NSString*)accessToken handler:(completionHandler)handler {
    ...
    NSURLSessionDataTask * dataTask = [manager GET:[NSString stringWithFormat:@"https://graph.qq.com/oauth2.0/me?access_token=%@&unionid=1",accessToken] parameters:nil progress:^(NSProgress * _Nonnull uploadProgress) {} success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSString *responseStr = [responseObject mj_JSONString];
        //是不是正确的数据
        if([responseStr rangeOfString:@"callback"].location == NSNotFound) {
            //主线程执行回调
            dispatch_async(dispatch_get_main_queue(), ^{
                handler(nil,[[MError alloc] initWithCode:E_HTTP_ERROR_QQ_UUID_ERROR statsMsg:@"获取失败"]);
            });
            return ;
        }
        //获取{}之间的部分
        ...
        NSString *dicStr= [array[1] componentsSeparatedByString:@"}"][0];
        //用,隔开 取出unionid
        for (NSString * tempStr in [dicStr componentsSeparatedByString:@","]) {
            if([NSString isBlank:tempStr])
                continue;
            NSArray *valueArr = [tempStr componentsSeparatedByString:@":"];
            if(valueArr.count != 2)
                continue;
            if([valueArr[0] rangeOfString:@"unionid"].location == NSNotFound)
                continue;
            //如果有""就去掉
            NSString *uuid = valueArr[1];
            NSArray *array = [uuid componentsSeparatedByString:@"\""];
            uuid = [array componentsJoinedByString:@""];
            //主线程执行回调
            dispatch_async(dispatch_get_main_queue(), ^{
                handler(uuid,nil);
            });
...
+ (NSURLSessionDataTask*)socialLogin:(NSString *)social_id unionId:(NSString*)union_id media_type:(int)media_type token:(NSString *)token expires_in:(NSString *)expires_in client_type:(NSString *)client_type name:(NSString *)name avatar_url:(NSString *)avatar_url handler:(completionHandler)handler {
    ...
    [params setObject:union_id forKey:@"union_id"];
