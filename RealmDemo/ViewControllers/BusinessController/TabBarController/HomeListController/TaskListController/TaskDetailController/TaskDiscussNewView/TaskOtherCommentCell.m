...
- (void)dataDidChange {
    ...
    self.timeLabel.text = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",(long)currDate.year,currDate.month,currDate.day,currDate.hour,currDate.minute];
...
- (IBAction)repClicked:(id)sender {
    if(self.delegate && [self.delegate respondsToSelector:@selector(TaskOtherRepClicked:)]) {
        [self.delegate TaskOtherRepClicked:self.data];
    }
}
