...
- (void)dataDidChange {
    ...
    self.finishTime.text = [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",(long)createDate.year,createDate.month,createDate.day,createDate.hour,createDate.minute];
    ...
    if(model.status == 1) {
        ...
        //是否超过了任务结束时间
        titleColor = timeDate > 0 ? [UIColor redColor] : [UIColor colorWithRed:10/255.f green:185/255.f blue:153/255.f alpha:1];
        titleStr = timeDate > 0 ? @"超" : @"余";
        timeDate = llabs(timeDate);
        int day = (int)timeDate / (24 * 60 * 60);
        int hour = (int)timeDate % (24 * 60 * 60) / (60 * 60);
        int minute = (int)timeDate % (60 * 60) / 60;
        titleStr = [NSString stringWithFormat:@"%@：%d天%d时%d分",titleStr,day,hour,minute];
    }
    self.detailLabel.text = titleStr;
