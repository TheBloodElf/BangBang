...
- (void)biaojiClicked:(UIBarButtonItem*)item {
    ...
    UIAlertAction *ok = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        for (PushMessage *pushMessage in array) {
            //本地推送只读取现在之前的
            if(pushMessage.id.doubleValue > 0)
                if(pushMessage.addTime.timeIntervalSince1970 > [NSDate date].timeIntervalSince1970)
                    continue;
            pushMessage.unread = NO;
            [_userManager updatePushMessage:pushMessage];
        }
    }];
    [alertVC addAction:cancle];
    [alertVC addAction:ok];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(tableView.editing == YES) return;
    ...
    if([message.type isEqualToString:@"COMPANY"]) {
        ...
        if(sharedCalendar.repeat_type == 0) {
            Calendar *tempTemp = sharedCalendar;
